    ...
    ssize_t transform_line(const char* line, size_t line_length, char* result_buffer, size_t result_buffer_size) {
        ...
        const char inserted_fragment[] = "KB";
        size_t inserted_size = sizeof(inserted_fragment) -1;
        size_t return_length = line_length;
        for(size_t i = 0; i< return_length; ++i) {
            if(static_cast<unsigned>(result_buffer[i] - '0') % 2 == 0) {
                memmove(&result_buffer[i + inserted_size-1],&result_buffer[i], sizeof(return_length - i));
                if(result_buffer_size - return_length < 2) {
                    return -1;
                }
                memcpy(&result_buffer[i], inserted_fragment, inserted_size);
                i += inserted_size - 1;
                return_length += inserted_size -1;
            }
        }
